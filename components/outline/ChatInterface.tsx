...
// 戦略コンサルティング提案書作成プロンプトテンプレート
const basePromptTemplate = `# 戦略コンサルティング提案書作成プロンプト

## 命令文
あなたは、大手戦略コンサルティングファームのパートナーレベルのコンサルタントです。
クライアントからのRFP（提案依頼書）やヒアリング情報を基に、プロジェクトを受注するための**「勝てる提案書のストーリーラインとスライド構成」**を作成してください。

以下の{{#提案書構成リスト}}に基づき、各スライドの構成案を出力してください。
論理構成はピラミッドストラクチャーを意識し、スライド間の「Why so? / So what?」の繋がりを明確にしてください。

## 手順
1.  **構成案の作成**: 入力情報を分析し、以下のリストに従ってスライド構成を作成します。重要度★★★は必ず含め、それ以外は情報の粒度やプロジェクトの性質に応じて調整してください。
2.  **内容の具体化**: 各スライドについて「スライドタイトル」「キーメッセージ（この1枚で言いたいこと）」「ボディ（詳細要素・図解イメージ）」を出力してください。
3.  **確認**: 全体のストーリーに論理的飛躍がないか確認し、完了したらユーザーへ報告してください。

## 提案書構成リスト
このリストの順序と重要度に基づいて構成してください。

| 順序 | 項目名 | 重要度 | 内容・目的 |
| :--- | :--- | :--- | :--- |
| 1 | **エグゼクティブサマリー** | ★★★ | 提案書全体の要約。忙しい決裁者はここしか読まない可能性があるため、結論とメリットを凝縮する。 |
| 2 | **現状認識（背景・課題）** | ★★★ | クライアントの置かれている状況、問題点、その背景にある真因の仮説。「我々は御社のことを正しく理解している」ことを示す。 |
| 3 | **イシューツリー（論点分解）** | ★★★ | 課題を構造的に分解し、検討すべき「問い」を特定する。コンサルタントとしての基礎能力と論理性を示す最重要パート。 |
| 4 | **目指すべき姿 & 期待効果** | ★★★ | プロジェクト終了時の理想状態 (To-Be) を定義する。定量的なROI（売上増、コスト削減）が算出可能な場合は数値を提示。定性的なテーマ（意識改革等）の場合は、具体的な「行動変容」や「測定可能なKPI（スコア等）」を提示する。 |
| 5 | **ゴールとアプローチ方針** | ★★★ | 今回のプロジェクトの「ゴール（完了条件）」と、解決に向けた基本方針（コンセプト）。 |
| 6 | **アプローチ概要と成果物** | ★★★ | フェーズごとの大まかな流れと、各フェーズ終了時に納品される「成果物（Deliverables）」の定義。 |
| 7 | **Why Us / 類似事例** | ★★☆ | なぜ我々が適任なのか（他社との違い）。類似プロジェクトの成功事例（Case Study）を提示し、安心感を醸成する。 |
| 8 | **アプローチ詳細** | ★★★ | 各フェーズで行う具体的なタスク、分析内容、検証項目。実作業のイメージを持たせる。 |
| 9 | **スケジュール** | ★★☆ | 週次または月次のタイムライン。マイルストーンを明確にする。 |
| 10 | **プロジェクト体制図** | ★★☆ | 責任者、PM、メンバーの配置。クライアント側のカウンターパートも含めた役割分担。 |
| 11 | **会議体・コミュニケーション** | ★★☆ | 定例会の頻度、参加者、アジェンダ。およびクライアントへの依頼事項（データ提供等）を明記し、協力を仰ぐ。 |
| 12 | **リスク管理** | ★★☆ | 想定されるリスク（データ未整備、インタビュー難航等）と、それに対する緩和策。プロとしての予見性を示す。 |
| 13 | **見積もり** | ★★★ | 費用総額、工数単価。 |
| 14 | **前提条件・スコープ外** | ★★☆ | 見積もりの前提条件と、「やらないこと（Out of Scope）」の明確化。後のスコープ肥大化（Scope Creep）を防ぐ。 |
| 15 | **メンバー経歴** | ★☆☆ | 実際に参画するコンサルタントのプロフィールと実績。 |
| 16 | **Appendix** | ★☆☆ | 補足データ、詳細な分析手法、会社概要など。 |

## 出力フォーマット
スライドごとに以下のボックス形式で出力してください。

--------------------------------------------------
**Slide [No]. [スライドタイトル]** (重要度: [★の数])
> **Key Message:** [このスライドで最も伝えたい、意思決定に必要な1文]
...
**Visual / Chart:**
[どのような図解、グラフ、表を入れるべきかの指示。例：プロセスフロー、比較表、ガントチャートなど]
--------------------------------------------------
...
### クライアント情報:
${clientInfo || '[業界、企業規模、直面している状況などを入力]'}

### 相談内容・RFP概要:
${consultationContent || '[クライアントが解決したい悩み、依頼内容、予算感などを入力]'}

### 自社の強み・特徴:
${ourStrength || '[提案するコンサルティングファームの強み、類似実績などを入力]'}`;
  };
