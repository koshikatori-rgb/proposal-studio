...
### 言語化確認ページ（/proposal/[id]/review）のバグ

1. **スライドタイプの誤分類**
   - `inferSlideTypeFromTitle`の精度が低い
   - 「イシューツリー」が「アプローチ」セクションに表示される

2. **スライドcontentが空になる**
   - 抽出APIは正しくcontentを返しているが、表示に反映されていない可能性

対応方針は `.claude/plans/tender-greeting-token.md` に記載
